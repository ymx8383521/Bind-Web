...
from  named.forms import dns_recordForm,dns_xfrForm
...
class NamedSyncView(LoginRequiredMixin,View):
    model = dns_record
    def get(self, request, *args, **kwargs):
        ser=int(time.time())
        context={'msg':''}
        try:
            dns_record.objects.filter(type__icontains='SOA').update(refresh=300,retry=3000,expire=86400,minimum=86400,serial=ser,resp_person='admin')
            # dns_record.objects.filter(~Q(type__icontains='SOA')).update(refresh=None,retry=None,expire=None,minimum=None,serial=None,resp_person=None)
            context['msg']='master success'
...
class RbacListView(LoginRequiredMixin, TemplateView, get_pagerange):
    model = dns_xfr
    template_name = "named/rbac_list.html"
    per = 15
    paginate_by = 15
    befor_range_num = 5
    after_range_num = 5
    ...
    def get_context_data(self, **kwargs):
        context = super(RbacListView, self).get_context_data(**kwargs)
        search = self.request.GET.get("search_data", None)  # 获取搜索字段数据
        page_num = int(self.request.GET.get("page", 1))  # 获取page id

        if search:
            obj_list = dns_xfr.objects.filter(
                Q(zone__contains=search) | Q(client__contains=search)).order_by('id')  # 取搜索域名
        else:
            obj_list = dns_xfr.objects.get_queryset().order_by('id')
        ...
        context['page_range'] = self.get_pageranges(context["page_obj"])  # page_range分页列表数据
        context['object_list'] = context['page_obj'].object_list  # 授权数据

        return context

    def post(self, request):
        form = dns_xfrForm(request.POST)
        if form.is_valid():
            form.save()
            res = {'code': 0, 'result': '添加授权成功'}
        else:
            # form.errors会把验证不通过的信息以对象的形式传到前端，前端直接渲染即可
            res = {'code': 1, 'errmsg': form.errors}
        return JsonResponse(res, safe=True)
...
class RbacPermissionView(LoginRequiredMixin, DetailView):
    '''
    动作：getone, update, delete
    '''
    model = dns_xfr
    template_name = "named/rbac_detail.html"
    context_object_name = 'named'
    next_url = '/named/grants/'

    def get_context_data(self, **kwargs):
        self.keyword = self.request.GET.get('keyword', '').strip()
        context = super(RbacPermissionView, self).get_context_data(**kwargs)
        context['keyword'] = self.keyword
        return context
    ...
    def post(self, request, *args, **kwargs):
        pk = kwargs.get('pk')
        p = self.model.objects.get(pk=pk)
        form = dns_xfrForm(request.POST, instance=p)
        if form.is_valid():
            form.save()
            res = {"code": 0, "result": "更新成功", 'next_url': self.next_url}
        else:
